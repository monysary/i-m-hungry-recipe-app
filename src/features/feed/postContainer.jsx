...
export default function PostContainer({ recipes, comments, userId }) {
  ...
  useEffect(() => {fetchAllLikes()},[toggle])
              ...
              <div key={recipe.id}>
                <m.div
                  ...
                  <m.div variants={cardVariantsVertical}>
                    <PostCard recipe={recipe} comments={comments} userId={userId}  setToggle={setToggle}   recipeLikes={recipeLikes.find((like) => like.recipeId === recipe.id)} />
                  </m.div>
                </m.div>
              </div>
            ))}
